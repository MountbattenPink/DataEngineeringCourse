...
spark.sql("USE CATALOG data_engineering")
...
# its needed for consistency in tables during ingesting files
schema = StructType([
...
try:
    ...
    bronze_stream = (df.writeStream.format("delta").outputMode("complete").option("checkpointLocation", bronze_checkpoint_path).start(bronze_table_path))
    ...
    def send_bronze_to_silver():
        ...
        silver_streaming_df = (bronze_streaming_df.filter(col("price") > 0)
